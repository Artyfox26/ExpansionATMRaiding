class DecoderTablet extends ItemBase {
	...
	// ExpansionATM support: allow hacking ExpansionATMBase and ExpansionATM
	void StartHackServer(ItemBase HackingTarget, PlayerBase Hacker) {
		...
		if (hacker && hackingTarget) {
			...
			// ExpansionATM support: check if target is ExpansionATMBase or ExpansionATM
			bool isExpansionATM = hackingTarget.IsInherited(ExpansionATMBase) || hackingTarget.IsInherited(ExpansionATM);
			int hackTimeMs = 0;
			if (isExpansionATM) {
				hackTimeMs = GetHackTabletConfig().GetHackTime("ExpansionATM");
			} else {
				hackTimeMs = GetHackingModConfig().GetHackTime(hackingTarget.GetType());
			}
			m_HackTimeRemaining = hackTimeMs * 1000;

			if (hackingTarget.GetHackID() != this.GetHackID() || hackingTarget.GetHackID() == 0) {
				...
				if (hacker.GetIdentity() && GetHackTabletConfig().ScriptLogging) {
					Print("[ATMRAID][Raid] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") has started hacking " + hackingTarget.GetType() + " with ID: " + HackID + " at " + hackingTarget.GetPosition());
				}
				GetGame().AdminLog("[ATMRAID][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") has started hacking " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition());
			} else {
				...
				resumed = " has resumed";
				if (hacker.GetIdentity() && GetHackTabletConfig().ScriptLogging) {
					Print("[ATMRAID][Raid] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") has resumed hacking " + hackingTarget.GetType() + " with ID: " + GetHackID() + " at " + hackingTarget.GetPosition());
				}
				GetGame().AdminLog("[ATMRAID][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") has resumed hacking " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition());
			}
	...
	bool CheckHackInterrupt(ItemBase HackingTarget, PlayerBase Hacker) {
		...
		if (hacker && hacker.GetIdentity() && hackingTarget) {
			bool isExpansionATM = hackingTarget.IsInherited(ExpansionATMBase) || hackingTarget.IsInherited(ExpansionATM);
			HackableItem hackingData;
			if (isExpansionATM) {
				hackingData = GetHackTabletConfig().GetHackableItem("ExpansionATM");
			} else {
				hackingData = GetHackingModConfig().GetHackableItem(hackingTarget.GetType());
			}
			if (isExpansionATM) {
				// ExpansionATM: check if ATM is locked (custom logic, e.g. ATM locked state)
				ExpansionATMBase atm = ExpansionATMBase.Cast(hackingTarget);
				if (atm && !atm.IsLocked()) {
					IsInterrupted = true;
					GetGame().AdminLog("[ATMRAID][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of ATM at " + hackingTarget.GetPosition() + " Interrupted due to ATM being unlocked");
				}
			} else {
				...
				if (codelock && hackingData.Type != "") {
					if (!codelock.GetLockState()) {
						IsInterrupted = true;
						GetGame().AdminLog("[ATMRAID][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to codelock being unlocked");
					}
				} else {
					IsInterrupted = true;
					GetGame().AdminLog("[ATMRAID][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to codelock missing");
				}
			}
			// Missing Batteries
			if (!GetHackTabletConfig().CanHack(isExpansionATM ? "ExpansionATM" : hackingTarget.GetType(), this.CountBatteries())) {
				IsInterrupted = true;
				GetGame().AdminLog("[ATMRAID][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to Missing Batteries");
			}
			// Random Chance
			...
			if (DoInterrupt < InterruptChance) {
				IsInterrupted = true;
				GetGame().AdminLog("[ATMRAID][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to interrupt Chance");
			}
			// Tablet in hands
			if (hacker.GetItemInHands() != this && RequireInHands) {
				IsInterrupted = true;
				GetGame().AdminLog("[ATMRAID][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to tablet dropped from hands");
			}
			// Out of radius
			if (vector.Distance(hackingTarget.GetPosition(), hacker.GetPosition()) > HackingRadius) {
				IsInterrupted = true;
				GetGame().AdminLog("[ATMRAID][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to Hacker Leaving the hacking radius");
			}
			// Tablet ruined
			if (this.IsRuined()) {
				IsInterrupted = true;
				GetGame().AdminLog("[ATMRAID][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to Tablet Being Destroyed");
			}
			// Player disconnected
			if (hacker.IsPlayerDisconnected()) {
				IsInterrupted = true;
				GetGame().AdminLog("[ATMRAID][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to Player Disconnected");
			}
		} else {
			IsInterrupted = true;
			GetGame().AdminLog("[ATMRAID][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to invalid Target or Player");
		}
	...
	void HackCompleted(ItemBase HackingTarget, PlayerBase Hacker) {
		...
		if (hackingTarget && hacker) {
			...
			if (isExpansionATM) {
				ExpansionATMBase atm = ExpansionATMBase.Cast(hackingTarget);
				if (atm) {
					atm.UnlockATMServer(hacker, hackingTarget); // Custom unlock logic for ExpansionATM
					GetGame().AdminLog("[ATMRAID][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") Hacked ATM at " + hackingTarget.GetPosition());
				}
			} else {
				CodeLock codelock = CodeLock.Cast(hackingTarget.GetAttachmentByConfigTypeName("CodeLock"));
				if (codelock && hackingData.Type != "") {
					...
					#endif
					if (hacker.GetIdentity() && GetHackTabletConfig().ScriptLogging) {
						Print("[ATMRAID][Raid] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") Hacked  " + hackingTarget.GetType() + " with ID: " + GetHackID() + " at " + hackingTarget.GetPosition());
					}
					GetGame().AdminLog("[ATMRAID][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") Hacked  " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition());
				}
			}
	...
	void TurnOnTablet(){
		...
		if(!IsRuined()){
			SetObjectTexture(0, "ExpansionATMRaiding\\Data\\textures\\tablet_on_co.paa");
			SetObjectTexture( GetHiddenSelectionIndex("zbytek"), "ExpansionATMRaiding\\Data\\textures\\tablet_on_co.paa");
			SetObjectTexture( GetHiddenSelectionIndex("tablet_on"), "ExpansionATMRaiding\\Data\\textures\\tablet_on_co.paa");
			SetObjectTexture( GetHiddenSelectionIndex("tablef_off"), "ExpansionATMRaiding\\Data\\textures\\tablet_on_co.paa");
			SetObjectTexture( GetHiddenSelectionIndex("top"), "ExpansionATMRaiding\\Data\\textures\\tablet_on_co.paa");
			SetObjectTexture( GetHiddenSelectionIndex("background"), "ExpansionATMRaiding\\Data\\textures\\tablet_on_co.paa");
			SetObjectTexture( GetHiddenSelectionIndex("screen"), "ExpansionATMRaiding\\Data\\textures\\tablet_on_co.paa");
			SetObjectTexture( GetHiddenSelectionIndex("backscreen"), "ExpansionATMRaiding\\Data\\textures\\tablet_on_co.paa");
			SetObjectMaterial( GetHiddenSelectionIndex("backscreen"), "ExpansionATMRaiding\\Data\\textures\\Tablet_screen_on.rvmat" );
		}
	}
	...
	void TurnOffTablet(){
		...
		if(!IsRuined()){
			SetObjectTexture(0, "ExpansionATMRaiding\\Data\\textures\\Tablet_off_co.paa");
			SetObjectTexture( GetHiddenSelectionIndex("zbytek"), "ExpansionATMRaiding\\Data\\textures\\Tablet_off_co.paa");
			SetObjectTexture( GetHiddenSelectionIndex("tablet_on"), "ExpansionATMRaiding\\Data\\textures\\Tablet_off_co.paa");
			SetObjectTexture( GetHiddenSelectionIndex("tablef_off"), "ExpansionATMRaiding\\Data\\textures\\Tablet_off_co.paa");
			SetObjectTexture( GetHiddenSelectionIndex("top"), "ExpansionATMRaiding\\Data\\textures\\Tablet_off_co.paa");
			SetObjectTexture( GetHiddenSelectionIndex("background"), "ExpansionATMRaiding\\Data\\textures\\Tablet_off_co.paa");
			SetObjectTexture( GetHiddenSelectionIndex("screen"), "ExpansionATMRaiding\\Data\\textures\\Tablet_off_co.paa");
			SetObjectMaterial( GetHiddenSelectionIndex("backscreen"), "ExpansionATMRaiding\\Data\\textures\\Tablet_on.rvmat" );
		}
	}
	...
	void SendPlayerMessage(PlayerBase hacker, string heading, string text){
		...
		string Message = text;
		PlayerBase Hacker = PlayerBase.Cast(hacker);
		string NotificationIcon = "HackingExpansionATMRaidingMod/GUI/Images/hacking.paa";
